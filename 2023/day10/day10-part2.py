...
def move_in_pipe(pos1, pos2, previous_steps):
    choice1 = available_directions(pos1[0], pos1[1])
    choice2 = available_directions(pos2[0], pos2[1])
    for c in choice1:
        newpos1 = move_pos(pos1, c, previous_steps)
        if newpos1 is not None:
            break
    for c in choice2:
        newpos2 = move_pos(pos2, c, previous_steps)
        if newpos2 is not None:
            break
...
with open('p2_ex1.txt') as input_file:
    ...
    same_pos = False
    step = 0
    newpos_1 = move_pos(my_pos, choices[0], my_pos)
    newpos_2 = move_pos(my_pos, choices[1], my_pos)
    my_pos = [my_pos]
    my_pos.append(newpos_1)
    my_pos.append(newpos_2)
    step += 1
    while not same_pos:
        # print(my_pos)
        newpos_1, newpos_2 = move_in_pipe(newpos_1, newpos_2, my_pos)
        my_pos.append(newpos_1)
        my_pos.append(newpos_2)
        step += 1
        if newpos_1 == newpos_2:
            same_pos = True
input_file.close()
print(my_pos)
